...
class VisualizationManager {
  constructor() {
    ...
    this.renderThrottle = 16; // 限制 FPS 为 60
    // 语言切换重绘优化
    this.lastRedrawTime = 0;
    this.redrawThrottle = 300; // 语言切换重绘节流 300ms
    this.isRedrawing = false; // 防止重入
    ...
    try {
      ...
      switch (this.currentModule) {
        ...
        case 3:
          ...
          break;
        case 5:
          // 模块 5 的 Canvas - 使用 requestAnimationFrame 分批绘制
          this.scheduleModule5Redraw();
          break;
      }
    } finally {
      // 重置重入标志
  ...
  scheduleModule5Redraw() {
    // 使用 requestAnimationFrame 延迟绘制，避免阻塞语言切换动画
...
drawGridToCanvas(canvasId, width = 500, height = 300, gridSize = 40) {
  ...
  initModule4() {
    ...
    // 绑定效果选择和流场控制（只绑定一次）
    if (!this.module4EffectsBound) {
      this.bindModule4Effects();
      this.bindFlowControls();
      this.module4EffectsBound = true;
    }
    this.initParticles();
  ...
  fbm(x, y, z, octaves = 4, persistence = 0.5, lacunarity = 2) {
    ...
    // 使用预计算的值进行噪声叠加
    for (let i = 0; i < octaves; i++) {
      value += amplitudes[i] * this.perlinNoise3D(x * frequencies[i], y * frequencies[i], z * frequencies[i]);
    }
  ...
  setupModule4Parameters() {
    ...
    // 绑定参数控制事件（只绑定一次）
    if (!this.module4ControlsBound) {
      this.bindModule4Controls();
      this.module4ControlsBound = true;
    }
  }

  // 绑定模块 4 的控制事件（现在使用 HTML oninput 属性，不需要此方法）
  bindModule4Controls() {
    // 此方法已被弃用，所有控制通过 HTML oninput 属性直接调用 updateModule4Param()
    console.log('[Module4] bindModule4Controls is deprecated, using oninput instead');
  }
  ...
  renderNoiseMap(ctx, width, height, scale) {
    // 性能优化：使用 ImageData 直接操作像素
    const imageData = ctx.createImageData(width, height);
    const data = imageData.data;
    const octaves = this.module4Params.octaves;
    const persistence = this.module4Params.persistence;
    const lacunarity = this.module4Params.lacunarity;
    ...
    for (let y = 0; y < height; y++) {
      ...
      for (let x = 0; x < width; x++) {
        const noise = this.fbm(x * scale2D, y * scale2D, 0, octaves, persistence, lacunarity);
  ...
  renderTerrain(ctx, width, height, scale) {
    ...
    for (let y = 0; y < height; y++) {
      const yOffset = y * width;
      for (let x = 0; x < width; x++) {
        const h = this.fbm(x * scale2D, y * scale2D, 0, octaves, persistence, lacunarity);
  ...
  renderFlowField(ctx, width, height, scale) {
    ...
    if (this.showParticles) {
      ...
      for (let i = 0; i < particleCount; i++) {
        const p = particles[i];
        const v = this.perlinNoise3D(p.x * scale2D, p.y * scale2D, t);
        const angle = v * Math.PI * 4;
  ...
  initModule5() {
    // 性能优化：检查是否已绘制过
    if (this.module5Drawn) {
      return; // 已绘制过，直接返回
    }
    
    // 性能优化：延迟绘制，使用 requestAnimationFrame 分批执行
    this.module5DrawScheduled = false;
    this.scheduleModule5Draw();
  }
  
  // 调度绘制任务（避免阻塞主线程）
  scheduleModule5Draw() {
    if (this.module5DrawScheduled) return;
    this.module5DrawScheduled = true;
    
    // 使用 requestAnimationFrame 延迟到下一帧
    requestAnimationFrame(() => {
      this.drawComparisonBatch1(); // 第一批：优先级高的
      setTimeout(() => {
        this.drawComparisonBatch2(); // 第二批：次要的
        this.module5Drawn = true; // 标记已完成绘制
      }, 50); // 50ms 延迟，避免阻塞
    });
  }
  
  // 第一批绘制（立即可见的内容）
  drawComparisonBatch1() {
    this.drawWhiteNoiseCompare();
    this.drawPerlinNoiseCompare();
  }
  
  // 第二批绘制（可延迟的内容）
  drawComparisonBatch2() {
  ...
  drawPerlinNoiseCompare() {
    ...
    for (let y = 0; y < height; y++) {
      const yOffset = y * width;
      for (let x = 0; x < width; x++) {
        const noise = this.fbm(x * scale, y * scale, 0, octaves, persistence);
        const value = Math.floor((noise + 1) * 127.5);
        const index = (yOffset + x) * 4;
  ...
  drawNoiseTypes() {
    ...
    ctx.textAlign = 'center';
    const whiteNoiseLabel = this.currentLanguage === 'zh' ? '白噪声' : 'White Noise';
    ctx.fillText(whiteNoiseLabel, sectionWidth / 2, 30);
    ...
    ctx.fillStyle = '#ffffff';
    const perlinNoiseLabel = this.currentLanguage === 'zh' ? '柏林噪声' : 'Perlin Noise';
    ctx.fillText(perlinNoiseLabel, sectionWidth * 1.5, 30);
    ...
    ctx.fillStyle = '#ffffff';
    const simplexNoiseLabel = this.currentLanguage === 'zh' ? 'Simplex 噪声' : 'Simplex Noise';
    ctx.fillText(simplexNoiseLabel, sectionWidth * 2.5, 30);
  }
...
function updateParam(name, input) {
  updateModule4Param(name, input);
}

// 模块 4：更新参数（直接调用）
function updateModule4Param(name, input) {
  if (!window.visualizationManager) {
    console.error('VisualizationManager not initialized');
    return;
  }

  const vm = window.visualizationManager;
  ...
  vm.module4Params[name] = val;
  
  // 更新显示值
  const displayEl = document.getElementById(name + 'Val');
  if (displayEl) {
    const displayMapper = DISPLAY_MAP[name];
    displayEl.textContent = displayMapper ? displayMapper(val) : val;
  }
...
// 当前打开的帮助弹窗参数
let currentHelpParam = null;

// 显示参数帮助弹窗
function showHelpModal(paramName) {
  const vm = window.visualizationManager;
  if (!vm) {
    console.error('VisualizationManager not initialized');
    return;
  ...
  currentHelpParam = paramName; // 记录当前参数
  const currentLang = vm.currentLanguage || 'zh';
  
  // 使用翻译系统的翻译键
  const t = (key) => {
    const lang = window.languageSwitcher;
    if (lang && lang.translations) {
      return lang.translations[currentLang]?.[`module4.help.${paramName}.${key}`] || 
             lang.translations['zh']?.[`module4.help.${paramName}.${key}`] || '';
    }
    return '';
  };
  ...
  // 设置标题
  const title = t('title');
  document.getElementById('paramHelpTitle').textContent = title;
  
  // 生成内容 HTML
  const contentHtml = `
    <div class="param-help-section">
      <p>${t('desc')}</p>
    </div>
    
    <div class="param-help-section">
      <h4>${t('details') || '详细说明'}</h4>
      <ul>
        <li>${t('detail1')}</li>
        <li>${t('detail2')}</li>
        <li>${t('detail3')}</li>
    ...
    <div class="param-help-section">
      <h4>${t('formula') || '公式'}</h4>
      <div class="param-help-formula">${t('formula')}</div>
    </div>
    
    <div class="param-help-note">
      <i class="fas fa-lightbulb"></i> ${t('note')}
    </div>
  `;
  
  document.getElementById('paramHelpContent').innerHTML = contentHtml;
  
  // 显示弹窗
  const modal = document.getElementById('parameterHelpModal');
  modal.classList.add('active');
  
  // 绘制可视化
  setTimeout(() => {
    drawParamVisualization(paramName);
  }, 100);
}
  ...
  if (currentHelpParam) {
    ...
    if (vm) {
      ...
      const t = (key) => {
        const lang = window.languageSwitcher;
        if (lang && lang.translations) {
          return lang.translations[currentLang]?.[`module4.help.${currentHelpParam}.${key}`] || 
                 lang.translations['zh']?.[`module4.help.${currentHelpParam}.${key}`] || '';
        }
        return '';
      };
      ...
      // 重新绘制可视化
      setTimeout(() => {
        drawParamVisualization(currentHelpParam);
      }, 100);
    }
  }
});
...
function closeModal(modalId) {
  if (modalId === 'parameterHelpModal') {
    currentHelpParam = null;
  }
  // 调用原有的 closeModal 函数
...
// 绘制参数可视化
function drawParamVisualization(paramName) {
  const canvas = document.getElementById('paramHelpCanvas');
  if (!canvas) return;
  ...
  switch (paramName) {
    case 'scale':
      ...
      break;
    case 'octaves':
      drawOctavesVisualization(ctx, width, height);
      break;
    case 'persistence':
      drawPersistenceVisualization(ctx, width, height);
      break;
    case 'lacunarity':
      drawLacunarityVisualization(ctx, width, height);
      break;
  }
}

// 缩放可视化
function drawScaleVisualization(ctx, width, height) {
  const vm = window.visualizationManager;
  if (!vm) return;
  
  // 绘制两个对比图
  const sectionWidth = width / 2 - 20;
  
  // 小缩放（左侧）
  ctx.fillStyle = '#1a1d2e';
  ctx.fillRect(10, 40, sectionWidth, height - 50);
  for (let y = 40; y < height - 10; y++) {
    for (let x = 10; x < sectionWidth + 10; x++) {
      const noise = vm.fbm(x * 0.002, (y - 40) * 0.002, 0, 4, 0.5, 2);
      const value = Math.floor((noise + 1) * 127.5);
      ctx.fillStyle = `rgb(${value}, ${value}, ${value})`;
  ...
  ctx.font = '14px Arial';
  ctx.textAlign = 'center';
  ctx.fillText('Scale: 0.002 (Zoomed In)', sectionWidth / 2 + 10, 25);
  
  // 大缩放（右侧）
  ctx.fillStyle = '#1a1d2e';
  ctx.fillRect(sectionWidth + 30, 40, sectionWidth, height - 50);
  for (let y = 40; y < height - 10; y++) {
    for (let x = sectionWidth + 30; x < width - 10; x++) {
      const noise = vm.fbm(x * 0.02, (y - 40) * 0.02, 0, 4, 0.5, 2);
      const value = Math.floor((noise + 1) * 127.5);
      ctx.fillStyle = `rgb(${value}, ${value}, ${value})`;
      ctx.fillRect(x, y, 2, 2);
    }
  ...
  ctx.fillStyle = '#ffffff';
  ctx.fillText('Scale: 0.02 (Zoomed Out)', sectionWidth * 1.5 + 20, 25);
}

// 叠加层数可视化
function drawOctavesVisualization(ctx, width, height) {
  const vm = window.visualizationManager;
  if (!vm) return;
  ...
  for (let y = 40; y < height - 10; y++) {
    for (let x = 10; x < sectionWidth + 10; x++) {
      const noise = vm.fbm(x * 0.01, (y - 40) * 0.01, 0, 1, 0.5, 2);
      const value = Math.floor((noise + 1) * 127.5);
      ctx.fillStyle = `rgb(${value}, ${value}, ${value})`;
      ctx.fillRect(x, y, 2, 2);
    }
  ...
  ctx.fillStyle = '#ffffff';
  ctx.textAlign = 'center';
  ctx.fillText('1 Octave (Simple)', sectionWidth / 2 + 10, 25);
  ...
  for (let y = 40; y < height - 10; y++) {
    for (let x = sectionWidth + 30; x < width - 10; x++) {
      const noise = vm.fbm(x * 0.01, (y - 40) * 0.01, 0, 6, 0.5, 2);
      const value = Math.floor((noise + 1) * 127.5);
      ctx.fillStyle = `rgb(${value}, ${value}, ${value})`;
      ctx.fillRect(x, y, 2, 2);
    }
  ...
  ctx.fillStyle = '#ffffff';
  ctx.fillText('6 Octaves (Detailed)', sectionWidth * 1.5 + 20, 25);
}

// 持续度可视化
function drawPersistenceVisualization(ctx, width, height) {
  const vm = window.visualizationManager;
  if (!vm) return;
  ...
  for (let y = 40; y < height - 10; y++) {
    for (let x = 10; x < sectionWidth + 10; x++) {
      const noise = vm.fbm(x * 0.01, (y - 40) * 0.01, 0, 4, 0.2, 2);
      const value = Math.floor((noise + 1) * 127.5);
      ctx.fillStyle = `rgb(${value}, ${value}, ${value})`;
      ctx.fillRect(x, y, 2, 2);
    }
  ...
  ctx.fillStyle = '#ffffff';
  ctx.textAlign = 'center';
  ctx.fillText('Persistence: 0.2 (Smooth)', sectionWidth / 2 + 10, 25);
  ...
  for (let y = 40; y < height - 10; y++) {
    for (let x = sectionWidth + 30; x < width - 10; x++) {
      const noise = vm.fbm(x * 0.01, (y - 40) * 0.01, 0, 4, 0.8, 2);
      const value = Math.floor((noise + 1) * 127.5);
      ctx.fillStyle = `rgb(${value}, ${value}, ${value})`;
      ctx.fillRect(x, y, 2, 2);
    }
  ...
  ctx.fillStyle = '#ffffff';
  ctx.fillText('Persistence: 0.8 (Rough)', sectionWidth * 1.5 + 20, 25);
}

// 空隙度可视化
function drawLacunarityVisualization(ctx, width, height) {
  const vm = window.visualizationManager;
  if (!vm) return;
  ...
  for (let y = 40; y < height - 10; y++) {
    for (let x = 10; x < sectionWidth + 10; x++) {
      const noise = vm.fbm(x * 0.01, (y - 40) * 0.01, 0, 4, 0.5, 1.5);
      const value = Math.floor((noise + 1) * 127.5);
      ctx.fillStyle = `rgb(${value}, ${value}, ${value})`;
      ctx.fillRect(x, y, 2, 2);
    }
  ...
  ctx.fillStyle = '#ffffff';
  ctx.textAlign = 'center';
  ctx.fillText('Lacunarity: 1.5 (Continuous)', sectionWidth / 2 + 10, 25);
  ...
  for (let y = 40; y < height - 10; y++) {
    for (let x = sectionWidth + 30; x < width - 10; x++) {
      const noise = vm.fbm(x * 0.01, (y - 40) * 0.01, 0, 4, 0.5, 2.5);
      const value = Math.floor((noise + 1) * 127.5);
      ctx.fillStyle = `rgb(${value}, ${value}, ${value})`;
      ctx.fillRect(x, y, 2, 2);
    }
  ...
  ctx.fillStyle = '#ffffff';
  ctx.fillText('Lacunarity: 2.5 (Fragmented)', sectionWidth * 1.5 + 20, 25);
}
