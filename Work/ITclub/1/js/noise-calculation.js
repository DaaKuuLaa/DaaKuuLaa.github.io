...
class NoiseCalculationVisualization {
    ...
    _t(key) {
        const translations = {
            zh: {
                bilinear: '双线性插值',
                smoothstep: 'S(t)=6t⁵-15t⁴+10t³',
                dotProduct: 'dot(梯度， 距离):',
                xInterp: 'X 轴插值 lerp(·,·,S(u)):',
                yInterp: 'Y 轴插值 lerp(E,F,S(v)):',
                dragHint: '拖拽 P 点 或 点击「演示计算过程」'
            },
            en: {
                bilinear: 'Bilinear Interpolation',
                smoothstep: 'S(t)=6t⁵-15t⁴+10t³',
                dotProduct: 'dot(gradient, distance):',
                xInterp: 'X Interp lerp(·,·,S(u)):',
                yInterp: 'Y Interp lerp(E,F,S(v)):',
                dragHint: 'Drag P or click "Play Demo"'
            }
        };
        return translations[this.currentLanguage]?.[key] || translations.zh[key];
    }
    ...
    _drawFormulas(ctx) {
        ...
        // ① 标题 + 坐标（同行）
        draw(this._t('bilinear'), '#8eb8ff', true);
        ...
        draw(this._t('yInterp'), nC, false, nHL);
    ...
    _drawHint(ctx) {
        ...
        ctx.textAlign   = 'center';
        ctx.fillText(this._t('dragHint'), cx, cy);
        ctx.restore();
    }
