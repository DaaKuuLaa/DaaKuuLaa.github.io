...
drawGridToCanvas(canvasId, width = 500, height = 300, gridSize = 40) {
  ...
  toggleDotProductView() {
    ...
    if (btn) {
      btn.innerHTML = this.step3ViewMode === 'single' ? 
  ...
  renderTerrain(ctx, width, height, scale) {
    const imageData = ctx.createImageData(width, height);
    const data = imageData.data;
    const t = this.animTime * this.module4Params.speed * 0.00015;
    
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        const h = this.fbm(x * scale, y * scale, t, this.module4Params.octaves, this.module4Params.persistence, this.module4Params.lacunarity);
